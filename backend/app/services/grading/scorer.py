...
class DSAScorer:
    ...
    def __init__(
        self, features: CodeFeatures, code_lower: str, filename_lower: str
    ) -> None:
        self.f = features
    ...
    def _check_structural_integrity(self) -> None:
        ...
        if "linked" in self.filename_lower and not f.has_class:
            self.notes.append(
                "Pedagogical requirement: Linked List assignment requires 'class Node'."
            )
            self.score = max(0, self.score - 2)
            self.details.append(
                "SYSTEM: Penalty for not using Class for Linked List (-2 pts)"
            )

        # Queue should use deque, not pop(0)
        if (
            "queue" in self.filename_lower
            and not f.has_deque
            and "pop(0)" in self.code_lower
        ):
            self.notes.append(
                "Performance: Using pop(0) on list for Queue is O(n), use collections.deque O(1)."
            )

        # Recursion check
        if (
            any(x in self.filename_lower for x in ["recursion", "de_quy", "backtrack"])
    ...
    def _score_basic_algorithms(self) -> None:
        ...
        # Linear search — only when no advanced structures present
            ...
            self._add_score(
                sc.SCORE_BASIC_ALGO,
                "Linear Search",
                f"Linear Search (+{sc.SCORE_BASIC_ALGO} pts)",
            )
    ...
    def _score_intermediate_structures(self) -> None:
        ...
        if f.has_class and "next" in f.class_attrs:
            if f.has_prev:
                self._add_score(
                    sc.SCORE_DOUBLE_LINKED_LIST,
                    "Double Linked List",
                    f"Double Linked List (+{sc.SCORE_DOUBLE_LINKED_LIST} pts)",
                )
            else:
                self._add_score(
                    sc.SCORE_LINKED_LIST,
                    "Linked List",
                    f"Linked List (+{sc.SCORE_LINKED_LIST} pts)",
                )
        ...
        if "heapq" in f.imports:
            self._add_score(
                sc.SCORE_HEAP, "Heap/Priority Queue", f"Heap (+{sc.SCORE_HEAP} pts)"
            )
    ...
    def _score_intermediate_algorithms(self) -> None:
        ...
        if (
            f.has_div2
            and f.has_while_loop
            and f.comparison_count > 0
            and not f.has_recursion
        ):
            self._add_score(
                sc.SCORE_BINARY_SEARCH,
                "Binary Search",
                f"Binary Search (divide by 2) (+{sc.SCORE_BINARY_SEARCH} pts)",
            )
        ...
        if f.has_recursion:
            self._add_score(
                sc.SCORE_RECURSION,
                "Recursion",
                f"Recursion (+{sc.SCORE_RECURSION} pts)",
            )
            # Divide & Conquer only if recursion has 2+ recursive calls (true D&C pattern)
            if f.is_divide_conquer:
                self._add_score(
                    sc.SCORE_DIVIDE_CONQUER,
                    "Divide & Conquer",
                    f"Divide & Conquer pattern (+{sc.SCORE_DIVIDE_CONQUER} pts)",
                )
        ...
        # Recursive binary search (has both div2 and recursion)
            ...
            self._add_score(
                ...
                f"Binary Search recursive (+{sc.SCORE_BINARY_SEARCH} pts)",
            )
    ...
    def _score_sorting_algorithms(self) -> None:
        ...
        is_quick = (
            f.has_list_comp_filter
            or (f.has_swap and f.loop_count > 0)
            or "pivot" in self.code_lower
        )
        is_merge = (
            f.has_slicing or "mid" in self.code_lower or "merge" in self.code_lower
        )

        if is_quick and not is_merge:
            self._add_score(
                sc.SCORE_ADVANCED_SORT,
                "Quick Sort",
                f"Quick Sort (+{sc.SCORE_ADVANCED_SORT} pts)",
            )
        elif is_merge:
            self._add_score(
                sc.SCORE_ADVANCED_SORT,
                "Merge Sort",
                f"Merge Sort (+{sc.SCORE_ADVANCED_SORT} pts)",
            )
        elif "sort" in self.filename_lower:
            self._add_score(
                sc.SCORE_ADVANCED_SORT,
                "Advanced Sorting",
                f"Advanced Sorting (+{sc.SCORE_ADVANCED_SORT} pts)",
            )
    ...
    def _score_advanced_structures(self) -> None:
        ...
        if f.has_class and {"left", "right"}.issubset(f.class_attrs):
            self._add_score(
                ...
                f"BST/Tree (+{sc.SCORE_BST} pts)",
            )

        if f.has_class and "children" in f.class_attrs:
            self._add_score(
                sc.SCORE_TRIE, "Trie (Prefix Tree)", f"Trie (+{sc.SCORE_TRIE} pts)"
            )

        if (
            "networkx" in f.imports
            or "adj" in f.class_attrs
            or "graph" in f.class_attrs
        ):
    ...
    def _score_advanced_algorithms(self) -> None:
        ...
        if f.has_dp_var and (f.has_nested_loops or f.has_recursion):
            self._add_score(
                ...
                f"Dynamic Programming (+{sc.SCORE_DP} pts)",
            )

        if (f.has_deque or f.has_recursion) and (
            "visit" in self.code_lower or "seen" in self.code_lower
        ):
            self._add_score(
                sc.SCORE_BFS_DFS,
                "Graph Traversal (BFS/DFS)",
                f"BFS/DFS (+{sc.SCORE_BFS_DFS} pts)",
            )
        ...
        if (
            ...
            and f.loop_count > 0
            and (
                "backtrack" in self.code_lower or "undo" in self.code_lower or f.has_pop
            )
        ):
            self._add_score(
                sc.SCORE_BACKTRACKING,
                "Backtracking",
                f"Backtracking (+{sc.SCORE_BACKTRACKING} pts)",
            )

        if "heapq" in f.imports and any(
            x in self.code_lower for x in ["dist", "cost", "d[", "distance"]
        ):
            self._add_score(
                sc.SCORE_DIJKSTRA,
                "Dijkstra's Algorithm",
                f"Dijkstra (+{sc.SCORE_DIJKSTRA} pts)",
            )
        ...
        if f.has_greedy_pattern and f.loop_count > 0:
            self._add_score(
                sc.SCORE_GREEDY,
                "Greedy Algorithm",
                f"Greedy pattern detected (+{sc.SCORE_GREEDY} pts)",
            )
    ...
    def _check_algorithm_mismatch(self) -> None:
        ...
        if self._expects_n_log_n_sort() and self._implemented_n_squared_sort():
            self._add_algorithm_mismatch_note(
                "Quick/Merge Sort (O(n log n))", "Bubble/Insertion Sort (O(n^2))"
            )
            self._penalize_wrong_algorithm()

        if self._expects_binary_search() and self._implemented_linear_search():
            self._add_algorithm_mismatch_note(
                "Binary Search (divide and conquer)", "Linear Search (sequential)"
            )
            self._penalize_wrong_algorithm()

    def _expects_n_log_n_sort(self) -> bool:
        return (
            any(x in self.filename_lower for x in ["quick", "merge", "heap"])
            and "sort" in self.filename_lower
        )
    ...
    def _add_algorithm_mismatch_note(self, expected: str, actual: str) -> None:
        self.notes.append(
            f"Algorithm mismatch: Expected {expected} but implemented {actual}."
        )
...
def score_complexity(features: CodeFeatures) -> Tuple[int, Optional[str]]:
    ...
    if max_loop_depth == 1 and features.has_div2 and not features.has_recursion:
        return (
            SCORING_CONSTANTS.COMPLEXITY_DEFAULT_SCORE,
            "Performance: O(log n) optimal (10/10 pts)",
        )
    ...
    if features.has_recursion:
        ...
        if max_loop_depth >= 2:
            ...
            return (
                SCORING_CONSTANTS.COMPLEXITY_O_N2_SCORE,
                "Performance: Recursion with nested loops may be O(n² log n) (8/10 pts)",
            )
        if max_loop_depth >= 1:
            # Recursion + single loop → O(n log n) or O(n²)
            return (
                SCORING_CONSTANTS.COMPLEXITY_O_N2_SCORE,
                "Performance: Recursion with loop, check complexity (8/10 pts)",
            )
        # Pure recursion, single call per level → O(n)
        return (
            SCORING_CONSTANTS.COMPLEXITY_DEFAULT_SCORE,
            "Performance: O(n) linear recursion (10/10 pts)",
        )

    # Loop depth scoring
    complexity_scores = {
        4: (
            SCORING_CONSTANTS.COMPLEXITY_O_N4_SCORE,
            f"Poor performance: O(n^{max_loop_depth}) complexity is too high (2/10 pts)",
        ),
        3: (
            SCORING_CONSTANTS.COMPLEXITY_O_N3_SCORE,
            "Performance: O(n^3) complexity is quite slow (5/10 pts)",
        ),
        2: (
            SCORING_CONSTANTS.COMPLEXITY_O_N2_SCORE,
            "Performance: O(n^2) average (8/10 pts)",
        ),
    }
    ...
    return (
        SCORING_CONSTANTS.COMPLEXITY_DEFAULT_SCORE,
        "Performance: O(n) good (10/10 pts)",
    )
