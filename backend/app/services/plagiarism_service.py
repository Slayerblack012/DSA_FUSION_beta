...
class PlagiarismService:
    ...
    def calculate_similarity(self, fingerprint1: str, fingerprint2: str) -> float:
        ...
        try:
            ...
            set1 = (
                set(fingerprint1.split("|")) if "|" in fingerprint1 else {fingerprint1}
            )
            set2 = (
                set(fingerprint2.split("|")) if "|" in fingerprint2 else {fingerprint2}
            )
    ...
    def _ast_node_similarity_from_trees(
        self, tree1: Optional[ast.AST], tree2: Optional[ast.AST]
    ) -> float:
    ...
    def _structural_similarity_from_trees(
        self, tree1: Optional[ast.AST], tree2: Optional[ast.AST]
    ) -> float:
    ...
    async def check_intra_job_plagiarism(
        ...
        for i in range(len(results)):
            for j in range(i + 1, len(results)):
                ...
                if (
                    getattr(result1, "status", "") == "FLAG"
                    or getattr(result2, "status", "") == "FLAG"
                ):
                    continue
                ...
                if similar:
                    ...
                    for submission in similar:
                        ...
                        if historical_code and code:
                            ...
                            detailed_sim, breakdown = (
                                self.calculate_combined_similarity(
